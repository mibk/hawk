...
var aritFns = map[string]struct {
	narg int
	fn   func(io.Writer, []*value.Value) *value.Value
}{
...
func atan2(w io.Writer, vals []*value.Value) *value.Value {
	return value.NewNumber(math.Atan2(vals[0].Float64(), vals[1].Float64()))
}

func cos(w io.Writer, vals []*value.Value) *value.Value {
	return value.NewNumber(math.Cos(vals[0].Float64()))
}

func exp(w io.Writer, vals []*value.Value) *value.Value {
	return value.NewNumber(math.Exp(vals[0].Float64()))
}

func _log(w io.Writer, vals []*value.Value) *value.Value {
	return value.NewNumber(math.Log(vals[0].Float64()))
}

func sin(w io.Writer, vals []*value.Value) *value.Value {
	return value.NewNumber(math.Sin(vals[0].Float64()))
}

func sqrt(w io.Writer, vals []*value.Value) *value.Value {
